...
def construct_run_config(iterations_per_loop):
  ...
  raise NotImplementedError('tpu not implemented')
...
def main(argv):
  ...
  if FLAGS.mode != 'eval' and FLAGS.mode != 'eval_once':
    if params['train_with_low_level_api']:
      raise NotImplementedError('low level api not implemented')
  ...
  if FLAGS.mode == 'train':
    ...
    if not params['eval_with_low_level_api']:
      if FLAGS.device == 'gpu':
        ...
        raise NotImplementedError('TPU not implmemented')
        # eval_estimator = tpu_estimator.TPUEstimator(
        #     model_fn=ssd_model.ssd_model_fn,
        #     use_tpu=FLAGS.use_tpu,
        #     train_batch_size=FLAGS.train_batch_size,
    ...
    for ckpt in next_checkpoint(FLAGS.model_dir):
      ...
      try:
        ...
        coco_eval(predictions, current_epoch, current_step, summary_writer)
